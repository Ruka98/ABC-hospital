...
const REPORTS_BUCKET = process.env.SUPABASE_REPORTS_BUCKET
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (image_file && image_file.size > 0) {
      ...
      const fileKey = `patient_${patient_id}/${timestamp}_${safeName}`

      // Prefer Supabase Storage when configured (works on Vercel).
      if (supabaseAdmin && REPORTS_BUCKET) {
        ...
        image_filename = fileKey
      } else {
        // Fallback: local disk (OK for local dev; NOT persistent on serverless)
        if (!existsSync(UPLOAD_DIR)) {
          await mkdir(UPLOAD_DIR, { recursive: true })
        }
        image_filename = `${timestamp}_${safeName}`
